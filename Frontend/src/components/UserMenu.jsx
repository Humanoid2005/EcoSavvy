      ...
      <Menu.Button className="flex items-center">
        {user.picture && (
          <img 
            src={user.picture} 
            alt="Profile" 
            className="w-8 h-8 rounded-full ring-2 ring-[#0D9488] cursor-pointer hover:opacity-80 transition-opacity duration-200"
          />
        )}
      </Menu.Button>
      ...
      >
        <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right bg-white rounded-xl shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="px-4 py-3 border-b border-gray-100">
            <p className="text-sm font-semibold text-[#0D9488]">{user.name}</p>
            ...
            <Menu.Item>
              {({ active }) => (
                <button
                  onClick={logout}
                  className={`${
                    active ? 'bg-[#0D9488] text-white' : 'text-gray-700'
